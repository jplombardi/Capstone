...
public class LidarController : MonoBehaviour
{
    ...
    [SerializeField] float angularResolution = 1.0f; // angular resolution 360 / resolution = num samples
    [SerializeField] float rangeMeters = 30.0f; // Range of the Lidar sensor in meters
    ...
    private Vector3[] dataPoints; // stores distance taken at angle = resolution*[i]
    ...
    void Start()
    {
        ...
        dataPoints = new Vector3[numShots]; //instantiate the empty array
    ...
    private void FireLIDAR()
    {
        if (enabled) //If we're enabled, start shooting those lasers!
        {
            for (int i = 0; i < numShots; i++) //iterate through the full 360 at specified resolution
            {
                ...
                {
                    dataPoints[i] = hit.point; //store the hit distance.
    ...
    public Vector3 getVector3FromDataPoint(int dp)
    {
